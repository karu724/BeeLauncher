    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    '경고<br>이 계정은 로그인이 된 마지막 계정입니다.',
                    '런처를 사용하려면 하나 이상의 계정에 로그인해야 합니다. 로그아웃을 하면 이후 다시 로그인해야 합니다.<br><br>로그아웃을 하시겠습니까?',
                    '로그아웃',
                    '뒤로가기'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                '무언가 잘 못 되었네요',
                'Microsoft 로그아웃에 실패하였습니다. 다시 시도해주세요.',
                '확인'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">인게임 닉네임</div>
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            '전환실패<br>One or More Drop-in Mods',
                            err.message,
                            '확인'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/karu724/BeeLauncher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            ...
            settingsUpdateButtonStatus('다운로딩..', true)
        }
    } else {
