...
function showMainUI(data){
    ...
    refreshServerStatus()
    setTimeout(() => {
        document.getElementById('frameBar').style.backgroundColor = 'rgba(255, 255, 255, 1)'
...
async function validateSelectedAccount(){
    ...
    if(selectedAcc != null){
        ...
        if(!val){
            ...
            setOverlayContent(
                '로그인 새로고침 실패',
                ` <strong>${selectedAcc.displayName}</strong>계정 로그인을 새로 고칠 수 없습니다. ${accLen > 0 ? '다른 계정 선택 또는 ' : ''} 다시 로그인하십시오.`,
                '로그인',
                '다른 계정 선택'
            )
            setOverlayHandler(() => {
